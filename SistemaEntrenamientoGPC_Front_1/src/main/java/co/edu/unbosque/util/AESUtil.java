 ...
 * <p>
 * <strong>NOTAS DE SEGURIDAD:</strong><br>
 * - El uso de una llave (key) y vector de inicialización (IV) fijos únicamente debe hacerse en entornos de pruebas.<br>
 * - Para producción se recomienda generar llaves seguras y IV aleatorios por mensaje, gestionados mediante un KMS o almacén seguro.<br>
 * - Las llaves y el IV aquí se convierten directamente con {@code String#getBytes()} (charset por defecto); para mayor control usar UTF-8 explícito.<br>
 * - El algoritmo utilizado es AES de 128 bits siempre que la llave tenga longitud de 16 bytes. Si se requiere 192/256 bits se debe ajustar la longitud y políticas de cifrado del JRE.<br>
